...
nerf_synthetic：合成数据集，一共有八个场景，都是blender模型拍摄的图片，每个场景100张图片作为训练集，100张图片作为验证集，200张作为测试集，测试集除了rbg图片还包含了深度图。
...
def parse_args():
    ...
    parser.add_argument('--datadir', type=str, default=r'D:\yuzhm01\Desktop\nerf-windows\data\ggbond', help='这个路径下需要有poses_bounds.npy以及images文件夹')
    # 网络模型参数
    ...
    # 下面三个环绕场景设为True，前向场景False一般用ndc，但也可以不用，效果会差一些
    ...
    for i in trange(1, args.iters + 1):
        ...
        if i % args.i_valid == 0:
            ...
            with torch.no_grad():
                ...
                print('验证完成')
        # 当设置 --i_test 为空或为 0 时，不进行测试渲染以避免除零错误
        if args.i_test and args.i_test > 0 and i % args.i_test == 0:
            iter_save_folder = os.path.join(iters_folder, str(i))
            # 创建保存视频的文件夹
